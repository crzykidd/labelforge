...
import { addTextElement, deleteSelected, getCanvasJSON, initCanvas, loadCanvasJSON, } from '../editor/canvas';
function esc(s) {
    return s
        .replace(/&/g, '&amp;')
        .replace(/"/g, '&quot;')
...
export function mountTemplateEditor(root) {
  ...
  `;
    ...
    const btnAddText = root.querySelector('#btn-add-text');
    ...
    const fontSelect = root.querySelector('#font-select');
    const fontSizeInput = root.querySelector('#font-size');
    const statusEl = root.querySelector('#editor-status');
    ...
    async function initEditor(label) {
        labelMedia = label.id;
        const [w, h] = label.dots_printable;
        const canvasEl = root.querySelector('#fabric-canvas');
    ...
    Promise.all([getLabels(), getFonts()]).then(async ([labels, fonts]) => {
        ...
        if (dvs) {
            fontSelect.value = dvs.name;
            defaultFont = dvs.name;
        }
        if (isNew) {
            // New template: use newMedia from query params
            const label = labels.find(l => l.id === newMedia);
            if (!label) {
                showStatus(`Unknown label media: ${newMedia}`, 'error');
                return;
            }
            ...
            try {
                ...
                if (!label) {
                    showStatus(`Unknown label media: ${tmpl.label_media}`, 'error');
                    return;
                }
                await initEditor(label);
